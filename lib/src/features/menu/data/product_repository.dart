...
abstract class IProductRepository {
  Future<List<Product>> loadProductsByCategory({required int categoryId, page = 0, limit = 25});
...
class ProductRepository implements IProductRepository {
  ...
  final ISavableProductsDataSource _dbProductDataSource;

  ProductRepository({
    required IProductDataSource networkProductDataSource,
    required ISavableProductsDataSource dbProductDataSource,
  }) : _networkProductDataSource = networkProductDataSource,
       _dbProductDataSource = dbProductDataSource;

  @override
  Future<List<Product>> loadProductsByCategory({required int categoryId, page = 0, limit = 25}) async {
    var dtos = <ProductDto>[];
    try {
      dtos = await _networkProductDataSource.loadProducts(categoryId: categoryId, page: page, limit: limit);
      _dbProductDataSource.saveProducts(products: dtos);
    } catch (_) {
      dtos = await _dbProductDataSource.loadProducts(categoryId: categoryId, page: page, limit: limit);
    }
